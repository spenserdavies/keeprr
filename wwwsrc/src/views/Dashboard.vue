<template>
  <div class="dashboard text-success" id="top">
      ...
      <div class="row mt-5">
        <div class="col-7 mx-auto text-center">
          <form v-if="keepForm" @submit.prevent="addKeep">
            ...
            <button
              type="submit"
              class="btn btn-primary float-right m-2 text-secondary border-secondary"
            >Submit</button>
            <button
              class="btn btn-secondary float-right m-2 text-primary border-secondary"
              @click="clearKeepForm"
            >Cancel</button>
          </form>
        </div>
    ...
    <div class="container-fluid mb-5">
      <div class="row">
        <div
          class="col-5 mx-auto border p-3 bg-danger border-primary text-center shadow title-shadow"
        >
          <h3>My Keeps</h3>
        </div>
      </div>
      <div class="row">
        <div class="col-5 mx-auto d-flex justify-content-between">
          ...
          <button
            v-if="viewKeeps == false"
            class="btn btn-success float-right m-2"
            @click="viewKeeps = true"
          >Show</button>
          <button
            v-if="viewKeeps == true"
            class="btn btn-secondary text-primary float-right m-2"
            @click="viewKeeps = false"
          >Hide</button>
        </div>
      </div>
    ...
    <div class="container-fluid mt-5">
      ...
      <div class="row">
        <div class="col-5 mx-auto d-flex justify-content-between">
          <button
            class="btn btn-primary ml-0 my-2"
            @click="vaultForm = true ; keepForm = false ; viewKeeps = false"
          >New</button>
          <button
            v-if="viewVaults == false"
            class="btn btn-success float-right m-2"
            @click="viewVaults = true"
          >Show</button>
          <button
            v-if="viewVaults == true"
            class="btn btn-secondary text-primary float-right m-2"
            @click="viewVaults = false"
          >Hide</button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    myKeeps() {
      return this.$store.state.myKeeps
        .filter((keep) => keep.userId == this.$auth.user.sub)
        .splice(0, 4);
    },
    vaults() {
